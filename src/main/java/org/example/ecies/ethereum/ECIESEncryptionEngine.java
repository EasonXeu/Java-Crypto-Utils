...
import static org.example.Base64Utils.encodeHexString;

/**
 * This class has been inspired by the <code>IESEngine</code> implementation in Bouncy Castle. It
...
public class ECIESEncryptionEngine {
    ...
    private ECIESEncryptionEngine(CipherParameters pubParam, CipherParameters privParam, PublicKey ephPubKey, byte[] iv) {
    ...
    public static ECIESEncryptionEngine forDecryption(
        final BCECPrivateKey privKey,
        final BCECPublicKey ephPubKey,
        final byte[] iv) {

        // Create parameters.
        final ECDomainParameters ecDomainParameters = Utils.getECDomainParameters();
        final CipherParameters pubParam = new ECPublicKeyParameters(ephPubKey.getQ(), ecDomainParameters);
        final CipherParameters privParam = new ECPrivateKeyParameters(privKey.getD(), ecDomainParameters);

        return new ECIESEncryptionEngine(pubParam, privParam, ephPubKey, iv);
    }
     ...
     */
    public static ECIESEncryptionEngine forEncryption(final BCECPublicKey pubKey) throws Exception {
        // Create an ephemeral key pair for IES whose public key we can later append in the message.
        final KeyPair ephemeralKeyPair = Utils.generateKeyPair();
        final BCECPublicKey ephemeralPublicKey = (BCECPublicKey) ephemeralKeyPair.getPublic();
        final BCECPrivateKey ephemeralPrivateKey = (BCECPrivateKey) ephemeralKeyPair.getPrivate();
        // Create random iv.
        final byte[] ivb = Utils.randomBytes(CIPHER_BLOCK_SIZE);
        System.out.println(encodeHexString(ivb));
        // Create parameters.
        final CipherParameters pubParam =
            new ECPublicKeyParameters(pubKey.getQ(), Utils.getECDomainParameters());
        final CipherParameters privParam =
            new ECPrivateKeyParameters(ephemeralPrivateKey.getD(), Utils.getECDomainParameters());

        return new ECIESEncryptionEngine(pubParam, privParam, ephemeralKeyPair.getPublic(), ivb);
    }
...
//  }
    public byte[] encrypt(final byte[] in) throws InvalidCipherTextException {
        return encrypt(in, 0, in.length, null);
    }
    ...
    public byte[] decrypt(final byte[] in)
        throws InvalidCipherTextException {
        return decrypt(in, 0, in.length, null);
    }
